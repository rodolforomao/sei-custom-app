...
const getMockingRoutes = () => {
  ...
  routes.push({ id: routesId.createLabel.id, url: "http://teste.mock/labels", body: `{"idBoard":"@{}", "name":"@{label.name}", "color": "@{label.color}"}`, verb: "POST", response: JSON.stringify(response) });
  routes.push({ id: routesId.updateLabel.id, url: "http://teste.mock/labels/${labelID}", body: `{"name":"@{label.name}", "color": "@{label.color}"}`, verb: "PUT", response: JSON.stringify(response) });
  return routes;
};
